...
- **Placement follows each layout.** On game pages the script clones Steam's action container to open a separate row below it. Bundles and packages have **two** purchase areas — one at the top and another after the list of what the bundle includes — and the three buttons go into their own row inside **each** of them. That matters beyond convenience: Steam marks the top one `tablet_hidden`, so on narrow screens it is the bottom one, and only the bottom one, that you get.
...
- **La colocación sigue cada maquetación.** En las páginas de juego el script clona el contenedor de acciones de Steam para abrir una fila aparte debajo. Los bundles y paquetes tienen **dos** zonas de compra —una arriba y otra al final, tras el listado de lo que incluye el lote— y los tres botones van en su propia fila dentro de **cada** una. No es solo comodidad: Steam marca la de arriba como `tablet_hidden`, así que en pantallas estrechas la que queda, y la única, es la de abajo.
